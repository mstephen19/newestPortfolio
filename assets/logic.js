...
const projects = [
  {
    ...
    description:
      'An app designed for those interested in donating to a charity. Users can find local charities, as well as global charities. Their most recently clicked results are saved to the history section.',
  ...
  {
    title: 'Express Notes',
    description: 'Take, save, and delete notes using this Express.js API!',
    image: './assets/images/noteTaker.png',
    github: 'https://github.com/mstephen19/noteTaker',
    link: 'https://salty-plateau-23106.herokuapp.com/',
  },
  {
    title: 'omegleChatBot',
    description:
      "Chat bot that spams a message to hundreds of people within minutes. The bot takes the user's message, automatically sends it to a stranger on omegle.com, disconnects from the chat, and starts all over again.",
  ...
  },
  {
    ...
    description:
      'A very small JavaScript library that makes certain common tasks easier to achieve.',
  ...
  },
  {
    ...
    description:
      'Generate a random secure password based on specific parameters that you provide.',
  ...
  },
  {
    title: 'CodeQuiz',
    description:
      'A timed 20 question JavaScript quiz, in which the user is rewarded with time upon correctly answering, and docked time upon incorrectly answering. The user can locally save their highscores, along with their name.',
  ...
  },
  {
    ...
    description:
      'Create a color pallete for your website by randomly generating colors! Save your work with the "Save" button in the top right corner. Super mobile friendly.',
  ...
  },
  {
    ...
    description:
      'Generate a professional README file for your project with a badge based on just a few simple prompts.',
...
function insertToCarousel() {
  _(projects).forEach((i) => {
    el(carouselCells[i].children[0]).setAttributes({
    ...
    el(carouselCells[i].children[1].children[2].children[0]).setAttributes({
      href: projects[i].github,
      target: '_blank',
    });
    if (projects[i].link) {
      el(carouselCells[i].children[1].children[2].children[1]).setAttributes({
...
function pastePhoneNumber() {
  const number = _(
    document.querySelector('#phoneSection').children[1].getAttribute('data-ph')
  ).reverseString();
  el(this).hide();
